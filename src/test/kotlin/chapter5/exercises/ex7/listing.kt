...
class Exercise7 : WordSpec({
    ...
    fun <A, B> Stream<A>.map(f: (A) -> B): Stream<B> =
        this.foldRight({ empty() }, { item, accum  -> Cons({ f(item) }, accum) })
    //end::map[]

    //tag::filter[]
    fun <A> Stream<A>.filter(f: (A) -> Boolean): Stream<A> =
        this.foldRight({ empty() }, { item, accum  -> if (f(item)) Cons({ item }, accum) else accum() })
    //end::filter[]

    //tag::append[]
    fun <A> Stream<A>.append(sa: () -> Stream<A>): Stream<A> =
        this.foldRight(sa) { item, accum -> Cons({ item }, accum) }
    //end::append[]

    //tag::flatmap[]
    fun <A, B> Stream<A>.flatMap(f: (A) -> Stream<B>): Stream<B> =
        this.foldRight({ empty() }, { item, accum  -> f(item).append(accum) })
    //end::flatmap[]
